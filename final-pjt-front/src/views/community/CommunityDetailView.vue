<template>
  <div>
    ...
    <router-link :to="{ name: 'CommunityView' }" class="mt-5 list">
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-arrow-left-square-fill yellow" viewBox="0 0 16 16">
    ...
    <div v-if="post" class="d-flex m-5">
      ...
      <div class="m-5 text-start">
        ...
        <p v-if="post.likes_count">{{ post.liked_users[0] }}님<span v-if="post.likes_count - 1"> 외 {{ post.likes_count - 1 }}명</span>이 좋아합니다.</p>
        <div class="d-flex align-items-center">
          <h2 class="fw-bold mt-4">{{ post?.title }}</h2>
          <router-link class="white" v-if="post?.user === user.id" :to="{ name: 'CommunityUpdateView', params: { id: post.id } }">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-eraser-fill ms-4 mt-3" viewBox="0 0 16 16">
              <path d="M8.086 2.207a2 2 0 0 1 2.828 0l3.879 3.879a2 2 0 0 1 0 2.828l-5.5 5.5A2 2 0 0 1 7.879 15H5.12a2 2 0 0 1-1.414-.586l-2.5-2.5a2 2 0 0 1 0-2.828l6.879-6.879zm.66 11.34L3.453 8.254 1.914 9.793a1 1 0 0 0 0 1.414l2.5 2.5a1 1 0 0 0 .707.293H7.88a1 1 0 0 0 .707-.293l.16-.16z"/>
            </svg>
          </router-link>
          <button v-if="post?.user === user.id" @click="deletePost" class="btn p-0 d-flex white">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-trash3-fill ms-3 mt-3" viewBox="0 0 16 16">
    ...
    <div class="container text-start">
      <h4 class="fw-bolder">댓글 목록</h4>
      <p>총 댓글: {{post.comment_count}}개</p>
      <div v-if="post?.comment_set">
        <h5 v-for="comment in post.comment_set" :key="comment.id">
          > {{ comment.user_nickname }} -
          {{ comment.content }}
          <button v-if="comment.user === user.id" @click="deleteComment(comment.id)" class="btn" style="color: white;">
      ...
      <form @submit="createComment">
          <textarea v-model="commentText" placeholder="댓글을 입력하세요" class="form-control"></textarea>
          <button type="submit" class="btn purple rounded-pill mt-4">댓글 작성</button>
      </form>
    </div>
    <router-link :to="{ name: 'CommunityView' }" class="mt-5 list mb-5">
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-arrow-left-square-fill yellow" viewBox="0 0 16 16">
        <path d="M16 14a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12zm-4.5-6.5H5.707l2.147-2.146a.5.5 0 1 0-.708-.708l-3 3a.5.5 0 0 0 0 .708l3 3a.5.5 0 0 0 .708-.708L5.707 8.5H11.5a.5.5 0 0 0 0-1z"/>
      </svg> <p class="ms-2">목록으로</p>
    </router-link>
